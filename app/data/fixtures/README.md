# Frozen demo fixtures (PLAN.md Task 21: demo hardening)
...
from the real feeds (CLAUDE.md rule 3: "synthetic inputs, real mechanics").
...
are unreachable (conference wifi, air-gapped judging room, `docker compose up`
with no internet). `app/data/ingest/vuln_intel.py` always tries the live
feeds first; it only reads these files after both live calls fail, and it
records `kev_source="offline_fixture"` / `epss_source="offline_fixture"` on
the resulting `IngestRun.rejection_log` so the provenance trail is honest
about which run was live vs. frozen (CLAUDE.md rule 2: "provenance on every
number").
