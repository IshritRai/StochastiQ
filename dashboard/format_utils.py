"""Rupee formatting: lakh/crore convention.

A classic off-by-one boundary; see tests/test_format_utils.py for the
boundary cases this must get right.
"""
...
def format_inr(amount: float) -> str:
    ...
    if magnitude >= _LAKH:
        ...
        if round(lakh_value, 2) >= 100.0:
            # Rounding would display "100.00 L", but that's a full crore;
            # don't let the off-by-one land one unit short.
            return f"{sign}₹{magnitude / _CRORE:.2f} Cr"
