...
The automated guardrail suite (build-spec.md's R3 tests, `tests/test_engine_guardrails.py`)
...
these are actually found is by perturbing the live app -- changing a seed,
moving a slider back to where it started, comparing two pages that should
agree -- and checking the numbers, not by re-reading the code.
...
regress. Recorded here per CLAUDE.md's provenance rule, so the fix and its
reasoning survive independent of the commit that made it.
...
(correct -- see build-spec.md risk R3 on independence). But `run_scenario_impl`,
...
exactly 0 -- violating build-spec.md risk R3's explicit requirement that a
zero-effect override give delta-EAL exactly 0.

**Root cause:** a `st.slider` is integer-valued (0-100), so its default is
`round(measured_coverage * 100)` -- e.g. a true measured coverage of
